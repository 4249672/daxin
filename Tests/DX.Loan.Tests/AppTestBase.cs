...
namespace DX.Loan.Tests
{
    ...
    /// Provides methods to easily work with <see cref="LoanDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<LoanTestModule>
    {
        ...
        protected void UsingDbContext(Action<LoanDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LoanDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<LoanDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<LoanDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LoanDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LoanDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LoanDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LoanDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<LoanDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<LoanDbContext, Task<T>> func)
        {
            T result;
